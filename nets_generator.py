...
class NetsGenerator:
    def __init__(self, tokens, length, nets_amount=1):
        self.nets_amount = nets_amount
        self.tokens = tokens
        self.length = length
        self.edge_density = random.randint(1, 9) / 10
        self.nets = snakes.PetriNet("cycle_net")
...
if __name__ == "__main__":
    nets_generator = NetsGenerator(tokens=int(sys.argv[1]), length=int(sys.argv[2]), nets_amount=int(sys.argv[3]))
    nets_generator.build()
    # nets_generator.draw()
    save_to_file(nets_generator.nets, 'nets.pnml')
